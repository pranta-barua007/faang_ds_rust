...
// we can see that at every step we are only using 
// the previous step to calculate the current
// we can use 2 2D arrays to store previous and current step
// so we dont need to tract a 3D at every k step
...
// Space Complexity: O(2.n^2) -> drop the constant 2 -> O(n^2)
// where n^2 the size of the dp array we are using to store the results of subproblems
...
fn knight_prob(n: i32, k: i32, row: i32, column: i32) -> f64 {
    let mut prev_dp = vec![vec![0.0; n as usize]; n as usize];
    let mut current_dp = vec![vec![0.0; n as usize]; n as usize];
    
    prev_dp[row as usize][column as usize] = 1.0;
            ...
            for c in 0..n as usize {
                ...
                for (dr, dc) in KNIGHT_DIRECTIONS.iter() {
                    ...
                    if new_r >= 0 && new_r < n && new_c >= 0 && new_c < n {
                        prob += prev_dp[new_r as usize][new_c as usize];
                    }
                }
                current_dp[r][c] = prob / 8.0;
            }
        }
        std::mem::swap(&mut prev_dp, &mut current_dp);
        current_dp = vec![vec![0.0; n as usize]; n as usize];
    }

    let mut total_prob = 0.0;
    for r in 0..n as usize {
        for c in 0..n as usize {
            total_prob += prev_dp[r][c]; 
        }
    }
